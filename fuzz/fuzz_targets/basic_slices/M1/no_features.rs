...
use sherman_fuzz_utils::{BasicCommand, CommandSequence, ExtraRunnerInfo, Name, RunnerState};
...
fuzz_target!(|cmds: CommandSequence<TyName, BasicCommand<u8, Constant<UpperLetter>>>| {
    let cmds = cmds.map(|cmd| cmd.map_slice(|c| Constant(c.0)));

    let mut runner: RunnerState<u8, Constant<UpperLetter>, NoFeatures, 1> = RunnerState::init();
