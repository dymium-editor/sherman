...
//! ## Testing
//!
//! `RleTree` is a complicated data structure that makes judicious use of unsafe Rust. It's
...
//! operations (e.g., [`get`], [`insert`], [`drain`]) against a simpler implementation of the same
...
//! made. Or we can even represent the file content itself, using a [`Slice`] that is itself a
//! chunk of bytes with limited size (effectively just a rope).
//!
...
//! ## Naming
//!
//! This library is named after [General Sherman] (a particularly big tree).
//!
...
pub mod param;
pub use public_traits::{DirectionalAdd, DirectionalSub, Index, Slice};
pub use tree::{Builder, Drain, IntoIter, Iter, Removed, RleTree, SliceEntry, StableRef};

/// Implementation of [`Slice`] for *actual* run-length encoding - a run of identical values
///
