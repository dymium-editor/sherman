...
/// [`stable_ref`] or [`SliceRef`] for more).
///
...
impl<'t, I, S> SliceEntry<'t, I, S, param::EnableRefs> {
    ...
    /// the tree
    ///
    /// See [`SliceRef`] for more.
    pub fn stable_ref(&self) -> SliceRef<I, S> {
        SliceRef { handle: Cell::new(Some(self.slice.weak())) }
    }
}

/// Stable reference to a value in an [`RleTree`] to allow fetching after updates to the tree
///
/// New `SliceRef`s are produced by [`SliceEntry::stable_ref`].
///
/// # Semantics
///
/// A `SliceRef` will track a value as it evolves in the tree, referencing the same [`Slice`] after
    ...
    S: Slice<I>,
{
    ...
    /// `SliceRef`; you can, for example, `get` the entry for a slice ref in the new `RleTree`
    /// generated by removing a range.
    pub fn get<'t>(
        &self,
        tree: &'t RleTree<I, S, param::EnableRefs>,
    ) -> Option<SliceEntry<'t, I, S, param::EnableRefs>> {
        // note: If the tree has no root node, then this `SliceRef` is definitely not part of the
